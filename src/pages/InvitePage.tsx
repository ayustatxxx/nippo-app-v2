  ...
  useEffect(() => {
    const loadInviteData = async () => {
      try {
        ...
        // 既にデータが読み込まれている場合はスキップ（重複実行防止）
if (currentUser && group) {
  console.log('⏭️ 既にデータ読み込み済み、スキップ');
  setLoading(false);
  return;
}
...
console.log('📧 メールプレフィックス:', emailPrefix);

// 自動生成かどうかを判定するヘルパー関数
const isAutoGenerated = (name: string): boolean => {
  const lowerName = name.toLowerCase();
  const lowerPrefix = emailPrefix.toLowerCase();
  
  // メールプレフィックスと完全一致
  if (lowerName === lowerPrefix) return true;
...
// 優先順位1: profileData.fullName（プロフィール設定画面で登録）
if (!finalName && user?.profileData?.fullName) {
  finalName = user.profileData.fullName;
  console.log('✅ タイプ1: profileData.fullName =', finalName);
}

// 優先順位2: displayName（カスタム設定された場合のみ）
if (!finalName && user?.displayName && !isAutoGenerated(user.displayName)) {
  finalName = user.displayName;
  console.log('✅ タイプ2: displayName（カスタム） =', finalName);
} else if (user?.displayName && isAutoGenerated(user.displayName)) {
  console.log('⚠️ displayNameは自動生成っぽい:', user.displayName, '→ スキップ');
}

// 優先順位3: username（カスタム設定された場合のみ）
if (!finalName && user?.username && !isAutoGenerated(user.username)) {
  finalName = user.username;
  console.log('✅ タイプ3: username（カスタム） =', finalName);
} else if (user?.username && isAutoGenerated(user.username)) {
  console.log('⚠️ usernameは自動生成っぽい:', user.username, '→ スキップ');
}

// 最終的に名前を設定（既に入力されている場合は上書きしない）
if (finalName && !displayName) {
  setDisplayName(finalName);
  console.log('✅ 最終的な表示名:', finalName);
} else if (!finalName) {
...
<div style={{ marginBottom: '2.5rem' }}>
  {(() => {
  // プロフィール設定済みかチェック
  const hasProfile = currentUser?.profileData?.fullName;
  
  // メールのプレフィックスを取得
  const emailPrefix = currentUser?.email?.split('@')[0] || '';
  
  // 手動設定されたdisplayNameかチェック（useEffectと同じロジック）
  const hasValidDisplayName = 
    currentUser?.displayName && 
    !currentUser.displayName.startsWith(emailPrefix);
  
  // 手動設定されたusernameかチェック（useEffectと同じロジック）
  const hasValidUsername =
    currentUser?.username &&
    !currentUser.username.startsWith(emailPrefix);
  
  // 表示のみモード
  const isReadOnly = hasProfile || hasValidDisplayName || hasValidUsername;
